...
public abstract class FUBaseUIActivity extends AppCompatActivity implements View.OnClickListener,
    ...
    protected abstract void onFUActivityCreated();

    protected abstract void onFUSensorChanged(int rotation);
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            // float z = getEventHandler.values[2];
            if (Math.abs(x) > 3 || Math.abs(y) > 3) {
                if (Math.abs(x) > Math.abs(y)) {
                    onFUSensorChanged(x > 0 ? 0 : 180);
                } else {
                    onFUSensorChanged(y > 0 ? 90 : 270);
                }
            }
        }
    }
